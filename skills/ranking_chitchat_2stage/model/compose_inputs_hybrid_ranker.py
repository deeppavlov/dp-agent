...
class ComposeInputsHybridRanker(Component):
    ...
    def __call__(self, utterances_batch: list, history_batch: list, states_batch: Optional[list] = None):
        ...
        for i in range(len(utterances_batch)):
            full_context = (
                history_batch[i][-self.num_turns_const + 1 :] + [utterances_batch[i]]
                if not self.history_includes_last_utterance
                else history_batch[i][-self.num_turns_const :]
            )
    ...
    def _expand_context(self, context: List[str], padding: str) -> List[str]:
        ...
        if padding == "post":
            ...
            res = (
                ...
                else sent_list[: self.num_turns_const]
            )
            return res
        elif padding == "pre":
