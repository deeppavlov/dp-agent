...
class SchemaBase:
    ...
    def __init__(self, uuid: Optional[uuid4] = None, orm_instance: Optional[TMongoSchemaTypes] = None) -> None:
...
class Bot(User):
    def __init__(self, uuid: Optional[uuid4] = None, orm_instance: Optional[BotMongo] = None) -> None:
    ...
    async def get_or_create(cls):
        ...
        if bot_query:
            ...
            bot = cls.create_from_orm(orm_instance=orm_instance)
        else:
            bot = cls()
...
class Human(User):
    ...
    async def get_or_create(cls, user_telegram_id: str, device_type: Optional[str]):
        ...
        if human_query:
            ...
            human = cls.create_from_orm(orm_instance=orm_instance)
        else:
...
class HumanUtterance(Utterance):
    ...
    async def get_or_create(cls, text: str, user: Human, date_time: datetime):
        return cls(text=text, user=user, date_time=date_time)
...
class Dialog(SchemaBase):
    ...
    async def _create_orm_instance(self) -> None:
        ...
        await self.bot.save()
        map(lambda utterance: utterance.save(), self.utterances)
    ...
    async def get_or_create(cls, location: Any, user: Human, bot: Bot, channel_type: str):
        ...
        if dialog_query:
            ...
            dialog = cls.create_from_orm(orm_instance)
        else:
