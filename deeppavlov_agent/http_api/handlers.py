...
class ApiHandler:
    ...
    async def handle_api_request(self, request):
        ...
        if request.method == "POST":
            if "content-type" not in request.headers or not request.headers[
                "content-type"
            ...
            command_performed = await handle_command(
                payload, user_id, request.app["agent"].state_manager
            )
            ...
            return web.json_response(
                self.output_formatter(response["dialog"].to_dict())
            )
        ...
        if all(c in hexdigits for c in dialog_id):
            ...
            if not dialog_obj:
                raise web.HTTPNotFound(
                    reason=f"dialog with id {dialog_id} does not exist"
                )
    ...
    async def dialog_list(self, request):
        ...
        resp_dict = {"dialog_ids": list_ids, "next": next_offset_link}
