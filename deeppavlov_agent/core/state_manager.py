...
class StateManager:
    ...
    async def add_annotation_prev_bot_utt(
        self, dialog: Dialog, payload: Dict, label: str, **kwargs
    ):
    ...
    async def add_hypothesis_annotation(
        self, dialog: Dialog, payload: Dict, label: str, **kwargs
    ):
        ind = kwargs["ind"]
        dialog.utterances[-1].hypotheses[ind]["annotations"][label] = payload

    async def add_hypothesis_annotation_batch(
        self, dialog: Dialog, payload: Dict, label: str, **kwargs
    ):
    ...
    async def get_or_create_dialog(self, user_external_id, channel_type, **kwargs):
        return await Dialog.get_or_create_by_ext_id(
            self._db, user_external_id, channel_type
        )
