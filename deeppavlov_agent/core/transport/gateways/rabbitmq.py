...
class RabbitMQAgentGateway(RabbitMQTransportBase, AgentGatewayBase):
    ...
    def __init__(
        ...
        super(RabbitMQAgentGateway, self).__init__(
            config=config,
    ...
    async def _setup_queues(self) -> None:
        agent_namespace = self._config["agent_namespace"]
        in_queue_name = AGENT_QUEUE_NAME_TEMPLATE.format(
            agent_namespace=agent_namespace, agent_name=self._agent_name
        )
        self._in_queue = await self._agent_in_channel.declare_queue(
        ...
        await self._in_queue.bind(
            exchange=self._agent_in_exchange, routing_key=routing_key
        )
        ...
        if isinstance(message_in, ServiceResponseMessage) and self._on_service_callback:
            ...
            await self._loop.create_task(
                self._on_service_callback(
                    task_id=message_in.task_id, response=message_in.response
                )
            )
            ...
            await self._loop.create_task(
                self._on_service_callback(
                    task_id=message_in.task_id, response=message_in.exception
                )
            )
    ...
    async def send_to_service(self, service_name: str, payload: dict) -> None:
        ...
        message = Message(
            body=json.dumps(task.to_json()).encode("utf-8"),
        ...
        logger.debug(
            f'Published task {payload["task_id"]} with routing key {routing_key}'
        )

    async def send_to_channel(
        self, channel_id: str, user_id: str, response: str
    ) -> None:
        channel_message = ToChannelMessage(
        ...
        message = Message(
            body=json.dumps(channel_message_json).encode("utf-8"),
            delivery_mode=aio_pika.DeliveryMode.PERSISTENT,
            expiration=self._utterance_lifetime_sec,
        )

        routing_key = CHANNEL_ROUTING_KEY_TEMPLATE.format(
            agent_name=self._agent_name, channel_id=channel_id
        )
        await self._agent_out_exchange.publish(message=message, routing_key=routing_key)
...
class RabbitMQServiceGateway(RabbitMQTransportBase, ServiceGatewayBase):
    ...
    def __init__(self, config: dict, to_service_callback: Callable) -> None:
        super(RabbitMQServiceGateway, self).__init__(
            config=config, to_service_callback=to_service_callback
        )
        self._loop = asyncio.get_event_loop()
    ...
    async def _setup_queues(self) -> None:
        ...
        in_queue_name = SERVICE_QUEUE_NAME_TEMPLATE.format(
            agent_namespace=agent_namespace, service_name=self._service_name
        )
        ...
        for service_name in service_names:
            ...
            logger.info(
                f"Queue: {in_queue_name} bound to routing key: {service_routing_key}"
            )
    ...
    async def _process_tasks(self, tasks_batch: List[ServiceTaskMessage]) -> bool:
        ...
        try:
            responses_batch = await asyncio.wait_for(
                self._to_service_callback(payloads), self._utterance_lifetime_sec
            )

            results_replies = []

            for i, response in enumerate(responses_batch):
                results_replies.append(self._send_results(tasks_batch[i], response))
...
class RabbitMQChannelGateway(RabbitMQTransportBase, ChannelGatewayBase):
    ...
    def __init__(self, config: dict, to_channel_callback: Callable) -> None:
        super(RabbitMQChannelGateway, self).__init__(
            config=config, to_channel_callback=to_channel_callback
        )
        self._loop = asyncio.get_event_loop()
        self._agent_name = self._config["agent_name"]
    ...
    async def _setup_queues(self) -> None:
        ...
        in_queue_name = CHANNEL_QUEUE_NAME_TEMPLATE.format(
            agent_namespace=agent_namespace,
            agent_name=self._agent_name,
        ...
        routing_key = CHANNEL_ROUTING_KEY_TEMPLATE.format(
            agent_name=self._agent_name, channel_id=self._channel_id
        )
        await self._in_queue.bind(
            exchange=self._agent_out_exchange, routing_key=routing_key
        )
        logger.info(f"Queue: {in_queue_name} bound to routing key: {routing_key}")
    ...
    async def _on_message_callback(self, message: IncomingMessage) -> None:
        ...
        await self._loop.create_task(
            self._to_channel_callback(
                message_to_channel.user_id, message_to_channel.response
            )
        )
        await message.ack()
    ...
    async def send_to_agent(
        ...
        message_from_channel = FromChannelMessage(
            agent_name=self._agent_name,
            channel_id=channel_id,
            user_id=user_id,
            utterance=utterance,
        ...
        message = Message(
            body=json.dumps(message_json).encode("utf-8"),
            delivery_mode=aio_pika.DeliveryMode.PERSISTENT,
            expiration=self._utterance_lifetime_sec,
        )
