...
async def run_users(url, payload, mnu, mxu):
    ...
    async with aiohttp.ClientSession() as session:
        for i in range(mnu, mxu + 1):
            ...
            for _ in range(0, i):
                ...
                tasks.append(
                    asyncio.ensure_future(
                        perform_test_dialogue(session, url, user_id, payload)
                    )
                )
            ...
            print(
                f"test No {i} finished: {max(times)} {min(times)} {mean(times)} {median(times)} "
...
if __name__ == "__main__":
    ...
    future = asyncio.ensure_future(
        run_users(args.url, payloads, args.minusers, args.maxusers)
    )
